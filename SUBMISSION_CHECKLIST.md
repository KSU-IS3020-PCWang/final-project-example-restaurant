# Final Submission Checklist

- [ ] My application runs from PyCharm without an unhandled error.
- [ ] My application includes approximately 5–10 meaningful functions.
- [ ] My application uses a list, dictionary, tuple, or another appropriate basic collection.
- [ ] My application uses conditionals and loops.
- [ ] My application reads from or writes to a CSV or JSON file.
- [ ] My application uses realistic input validation and `try/except` error handling.
- [ ] Each meaningful function has a clear docstring.
- [ ] Important logic blocks and non-obvious lines have useful comments.
- [ ] `README.md` is complete.
- [ ] `AI_USAGE.md` is complete.
- [ ] I pushed the commit `Initial project setup`.
- [ ] I pushed the commit `Original working version before AI improvement`.
- [ ] I pushed the commit `Final version after AI improvement`.
- [ ] All three required commits appear in the GitHub repository history.
- [ ] My final GitHub commit appears in the repository history.
- [ ] I downloaded the final repository ZIP.
- [ ] I renamed the ZIP `LastName_FirstName_IS3020_FinalProject.zip`.
- [ ] I uploaded the ZIP to D2L.
- [ ] I pasted my GitHub repository URL into the D2L submission comments.
